...
export function App() {
  ...
  const originatingMatch = useRef<string | null>(null);
  ...
  useEffect(()=>{if(matches && !selected) (matchButtons.current.get(originatingMatch.current ?? '') ?? resultRef.current)?.focus();},[matches,selected]);
  function search(event:React.FormEvent) {
    ...
    if(!catalog)return;
    originatingMatch.current=null;setPrompt('');setMatches(discover(catalog,goal,platform));
  }
