...
export function ApplicationGuide({entry,platform,onBack}:{entry:CatalogEntry;platform:Platform;onBack:()=>void}) {
  ...
  return <section className="application" aria-labelledby="application-title">
    ...
    <ol className="installation">
      <li><h3>Save the skill</h3><p>Download this small instruction file. Keep its name as <code>SKILL.md</code>.</p><a className="primary" href={import.meta.env.BASE_URL+guide.downloadPath} onClick={event=>{if(!currentGuide())event.preventDefault();}} download="SKILL.md">Download skill</a></li>
