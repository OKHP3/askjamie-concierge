...
**Tailwind CSS and Mermaid are not used.** The two interfaces use authored CSS and shared UI tokens. No Tailwind package/configuration or Mermaid package/diagram source is present. There is no application database, model API call, hosted private backend, Docker image, or separately deployed Python service in this repository. Python packages such as OpenAI, Anthropic and AWS SDKs are installed through the scanner's dependency graph; their presence does not mean the public product calls those services. The configured scanner uses its static analyzer.
